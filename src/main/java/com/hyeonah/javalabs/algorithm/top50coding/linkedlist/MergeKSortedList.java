package com.hyeonah.javalabs.algorithm.top50coding.linkedlist;
...
public class MergeKSortedList {
    ...
    public static void main(final String[] args) {
        ...
        final ListNode[] list = new ListNode[3];
        list[0] = l1;
        list[1] = l2;
        list[2] = l3;
        final MergeKSortedList mergeKSortedList = new MergeKSortedList();
    ...
    public ListNode mergeKLists(final ListNode[] list) {
        // 1. 오름차순 소팅
        final PriorityQueue<ListNode> queue = new PriorityQueue<>(Comp);
        final ListNode resultNode = new ListNode(0); // dummy data
        ListNode copyNode = resultNode;

        // list 3개를 넣는다!
        for (final ListNode node : list) {
            if(node != null) {
                queue.offer(node);
            }
        }
