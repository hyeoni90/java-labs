...
public class MergeInterval {
    ...
    public static void main(final String[] args) {
        final int[][] nums = {{2, 6}, {1, 3}, {8, 10}, {15, 18}};
        final List<Interval> intervals = new ArrayList<>();
        for (int i = 0; i < nums.length; i++) {
            intervals.add(new Interval(nums[i][0], nums[i][1]));
        }

        final List<Interval> result = new MergeInterval().merge(intervals);
    ...
    private void print(final List<Interval> intervals) {
        for (int i = 0; i < intervals.size(); i++) {
            final Interval in = intervals.get(i);
