...
//        future.get(); // get(): blocking call이기 때문에 get전에 작업들을 해야함!
        ...
        final CompletableFuture<Void> notExistReturnFuture = CompletableFuture.runAsync(() ->{
            System.out.println("Hello, " + Thread.currentThread().getName());
        });
        System.out.println(notExistReturnFuture.get());

        // 2-1. static factory method and `return이 있는 작업` supplyAsync
        final CompletableFuture<String> existReturnFuture = CompletableFuture.supplyAsync(() ->{
            System.out.println("Hello, " + Thread.currentThread().getName());
            return "Hello";
        });
        System.out.println(existReturnFuture.get());

        final CompletableFuture<String> callBackFuture = CompletableFuture.supplyAsync(() ->{
            System.out.println("Hello, " + Thread.currentThread().getName());
            return "Hello";
        }).thenApply((s) -> {
        ...
        final CompletableFuture<Void> nonCallBackFuture = CompletableFuture.supplyAsync(() ->{
            System.out.println("Hello, " + Thread.currentThread().getName());
            return "HELLO";
        }).thenRun(() -> {
            System.out.println(Thread.currentThread().getName());
        ...
        final CompletableFuture<Void> anotherThreadPoolCallBackFuture = CompletableFuture.supplyAsync(() ->{
            System.out.println("Hello, " + Thread.currentThread().getName());
            return "HELLO";
